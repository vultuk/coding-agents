...
Create a new local plan skeleton or enrich an existing one in place:
    ...
    python3 .agents/skills/execplan/scripts/manage_execplan_comment.py normalize \
      --input "$PLAN_FILE" \
      --output "$PLAN_FILE" \
...
Before posting a new or refreshed plan body, lint the visible content:
    ...
    python3 .agents/skills/execplan/scripts/manage_execplan_comment.py lint \
      --input "$PLAN_FILE"
...
Add the next unchecked slice before implementation starts:
    ...
    python3 .agents/skills/execplan/scripts/manage_execplan_comment.py record-slice \
      --input "$PLAN_FILE" \
      --output "$PLAN_FILE" \
      --title "Implement issue #$ISSUE" \
      --slice-id EP-002 \
...
Render a stable handoff body with the fixed marker `<!-- execplan:handoff -->`:
    ...
    HANDOFF_BODY=$(mktemp)
    python3 .agents/skills/execplan/scripts/manage_execplan_comment.py render-handoff \
      --issue "$ISSUE" \
...
After PR creation, update the managed issue comment again so the issue records the final traceability details:
    ...
    python3 .agents/skills/execplan/scripts/manage_execplan_comment.py set-delivery \
      --input "$PLAN_FILE" \
      --output "$PLAN_FILE" \
      --title "Implement issue #$ISSUE" \
