...
Then, if needed for execution, write that content into a runnable path in the active workspace before calling it. Do **not** assume `python .agents/skills/monthly-platform-report/scripts/monthly_platform_report.py ...` will work unless you have already confirmed that path exists in the current filesystem context.
...
```bash
python .agents/skills/monthly-platform-report/scripts/monthly_platform_report.py extract-month \
  --year 2026 \
  --month 4 \
...
```bash
python .agents/skills/monthly-platform-report/scripts/monthly_platform_report.py render \
...
```bash
python -m unittest .agents/skills/monthly-platform-report/tests/test_monthly_platform_report.py -v
python .agents/skills/monthly-platform-report/scripts/monthly_platform_report.py --help
```
